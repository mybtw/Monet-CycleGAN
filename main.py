...
class CycleGan(keras.Model):
    ...
    def train_step(self, batch_data):
        real_x, real_y = batch_data  # x- Monet y - real
...
cycle_gan_model.gen_G.load_weights('./weights')

# cycle_gan_model.gen_G.summary()
...
#     if i == 5:
#         break
#     prediction = gen_G(tf.image.decode_jpeg(img), training=False)[0].numpy()
#     prediction = (prediction * 127.5 + 127.5).astype(np.uint8)
#     im = PIL.Image.fromarray(prediction)
#     im.save("./images/" + str(i) + ".jpg")
#     i += 1
test_dataset = tf.data.Dataset.list_files("./test_images/*.jpg")
# чтение изображения из файла и декодирование его при помощи функции decode_image
test_dataset = test_dataset.map(lambda x: tf.io.read_file(x))
test_dataset = test_dataset.map(decode_image).batch(1)

for i, img in enumerate(test_dataset):
    prediction = gen_G(img, training=False)[0].numpy()
    prediction = (prediction * 127.5 + 127.5).astype(np.uint8)
    #img = (img[0] * 127.5 + 127.5).numpy().astype(np.uint8)
    pr = PIL.Image.fromarray(prediction)
    #im = PIL.Image.fromarray(img)
    #im.save("./source_images/" + str(i) + ".jpg")
    pr.save("./images/" + str(i) + ".jpg")
